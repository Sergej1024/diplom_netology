...
resource "null_resource" "public_access" {
  provisioner "local-exec" {
    command = "ANSIBLE_FORCE_COLOR=1 ansible-playbook -i kubespray/inventory/diplomcluster/inventory-${terraform.workspace}.ini ansible/supplementary_addresses_in_ssl_keys-${terraform.workspace}.yml -b -v"
  }
...
resource "null_resource" "kubespray" {
  provisioner "local-exec" {
    command = "ANSIBLE_FORCE_COLOR=1 ansible-playbook -i kubespray/inventory/diplomcluster/inventory-${terraform.workspace}.ini kubespray/cluster.yml -b -v"
  }
...
resource "null_resource" "kubectl" {
  provisioner "local-exec" {
    command = "ANSIBLE_FORCE_COLOR=1 ansible-playbook -i kubespray/inventory/diplomcluster/inventory-${terraform.workspace}.ini ansible/playbook-${terraform.workspace}.yml -b -v"
  }
