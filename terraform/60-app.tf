...
resource "null_resource" "app_deployment" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../app/manifests/00-deployment.yml", data.template_file.app_deployment.rendered)
  }
...
resource "null_resource" "jenkinsfile_stage" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../app/Jenkinsfile", data.template_file.jenkinsfile_stage.rendered)
  }
...
resource "null_resource" "jenkinsfile_prod" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../app/Jenkinsfile-prod", data.template_file.jenkinsfile_prod.rendered)
  }
...
resource "null_resource" "app" {
  provisioner "local-exec" {
    command = <<EOF
      kubectl --kubeconfig=./kubeconfig/config-${terraform.workspace} apply -f ../app/manifests/
    EOF
  }
