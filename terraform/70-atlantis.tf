...
resource "null_resource" "atlantis_manifest" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../atlantis/manifests/10-satatefulSet.yml", data.template_file.atlantis_manifest.rendered)
  }
...
resource "null_resource" "atlantis_repo_config" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../terraform/server.yaml", data.template_file.atlantis_repo_config.rendered)
  }
...
resource "null_resource" "atlantis" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      kubectl --kubeconfig=./kubeconfig/config-${terraform.workspace} apply -f ../atlantis/manifests/
    EOF
  }
...
resource "null_resource" "terraform_repo" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      hook_id=''
      repo_id=''
      repo_name=diplom_netology
      repo_name_git=diplom_netology.git
      ...
      if [[ "$repo_id" == "null" ]]; then
        curl -sS \
          ...
          -d '{"name":"'$repo_name'","description":"Netology DevOps cource diplom, terraform manifests","homepage":"https://github.com","private":false,"is_template":false}'
      fi
      ##########################################################################
