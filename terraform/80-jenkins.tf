...
resource "null_resource" "app_repo" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      hook_id=''
      repo_id=''
      repo_name=diplom-test-app
      repo_name_git=diplom-test-app.git
...
data "template_file" "diplom_test_app_stage_config" {
  template = file("${path.module}/templates/diplom-test-app-stage-config.tpl")
...
resource "null_resource" "diplom_test_app_stage_config" {
  count = (terraform.workspace == "prod") ? 1 : 0

  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../jenkins/jobs/diplom-test-app-stage/config.xml", data.template_file.diplom_test_app_stage_config.rendered)
  }

  triggers = {
    template = data.template_file.diplom_test_app_stage_config.rendered
  }
}

# -------------------------------------------------
# Формирование задач Jenkins по шаблону, со ссылкой на репозиторий тестового приложения
data "template_file" "diplom_test_app_prod_config" {
  template = file("${path.module}/templates/diplom-test-app-stage-config.tpl")
  ...
  depends_on = [
    null_resource.diplom_test_app_stage_config
  ]
}

# -------------------------------------------------
# Сохранение рендера шаблона в файл
resource "null_resource" "diplom_test_app_prod_config" {
  count = (terraform.workspace == "prod") ? 1 : 0

  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../jenkins/jobs/diplom-test-app-stage/config.xml", data.template_file.diplom_test_app_prod_config.rendered)
  }

  triggers = {
    template = data.template_file.diplom_test_app_prod_config.rendered
  }
}
...
resource "null_resource" "jenkins_credentials" {
  ...
  provisioner "local-exec" {
    command = format("cat <<\"EOF\" > \"%s\"\n%s\nEOF", "../jenkins/exported-credentials.xml", data.template_file.jenkins_credentials.rendered)
  }
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      create configmap jenkins-files \
        --from-file=credentials=../jenkins/exported-credentials.xml \
        --from-file=diplom-test-app-stage=../jenkins/jobs/diplom-test-app-stage/config.xml \
        --from-file=diplom-test-app-prod=../jenkins/jobs/diplom-test-app-prod/config.xml \
...
resource "null_resource" "jenkins" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      kubectl --kubeconfig=./kubeconfig/config-${terraform.workspace} apply -f ../jenkins/manifests/
    EOF
  }
